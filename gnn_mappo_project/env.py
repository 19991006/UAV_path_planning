...
class UAVEnvConfig:
    ...
    inter_agent_min_distance: float = 0.35
...
class MultiUAV2DEnv:
    ...
    def get_graph_obs(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        Returns:
            node_features:
                Array with shape [num_agents, node_dim]. Each node is one UAV:
                [LiDAR distances, ego velocity/acceleration, assigned-target range/bearing].
                This dimension is independent of num_agents.

            edge_index:
                Integer array with shape [2, num_edges]. edge_index[0] contains source
                node indices and edge_index[1] contains destination node indices. The first
                implementation uses a directed fully connected UAV graph without self-loops.

            edge_attr:
                Array with shape [num_edges, edge_dim]. For edge i -> j, features are:
                [dx_ij/world_size, dy_ij/world_size, distance_ij/communication_range,
                 relative_bearing_ij/pi].

        This method deliberately excludes q_i from the old flat observation. Teammate
        information is represented as edge features so the policy can be reused for
    ...
    def _build_agent_graph(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        Full connectivity is used for the first GNN version because edge count is
        deterministic for a fixed training N and the same model parameters still work
        for a different evaluation N. Later, this can be replaced by communication
        radius or k-nearest-neighbor edges.
        """
        src_list = []
        dst_list = []
        edge_attrs = []
